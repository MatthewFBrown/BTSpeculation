...
{
  "wheelPositions": [
    { "ticker": "AAPL", "type": "CSP", "strike": 170, "expiry": "2025-05-16", "premium": 2.50, "contracts": 1 },
    { "ticker": "TSLA", "type": "CC",  "strike": 250, "expiry": "2025-05-16", "premium": 3.80, "contracts": 2 }
  ],
  "stockPositions": [
  ...
  "tradeLog": [
    { "ticker": "AAPL", "strategy": "CSP", "entry": "2025-01-10", "exit": "2025-01-24", "pnl": 250,  "result": "win"  },
    { "ticker": "MSFT", "strategy": "CSP", "entry": "2025-01-15", "exit": "2025-02-21", "pnl": 180,  "result": "win"  },
    { "ticker": "TSLA", "strategy": "CC",  "entry": "2025-02-03", "exit": "2025-02-28", "pnl": -120, "result": "loss" },
    { "ticker": "AAPL", "strategy": "CC",  "entry": "2025-02-10", "exit": "2025-03-07", "pnl": 340,  "result": "win"  }
  ]
}
```
...
import { WHEEL_POSITIONS, STOCK_POSITIONS, TRADE_LOG } from '../../utils/mattCapitalData'
...
export default function MattCapital() {
  ...
  useEffect(() => { loadPrices() }, [])  // eslint-disable-line
  ...
  const stockCost = STOCK_POSITIONS.reduce((sum, p) => sum + p.entryPrice * p.shares, 0)

  const premiumCollected = WHEEL_POSITIONS.reduce((sum, p) => sum + p.premium * p.contracts * 100, 0)
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-800/70 backdrop-blur-sm rounded-xl ring-1 ring-white/[0.06] p-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-100 tracking-tight">MATT Capital</h1>
            <p className="text-sm text-slate-400 mt-1">Wheel strategy · Options · Equities</p>
          </div>
          <button
            onClick={loadPrices}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
          {[
            ...
            { label: 'Total Return',     value: fmtPct(totalPct),             color: totalPct >= 0 ? 'text-emerald-400' : 'text-red-400' },
            { label: 'Wheel Win Rate',   value: `${winRate.toFixed(0)}%`,     color: 'text-blue-400' },
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.05]">
                {['Ticker','Type','Strike','Expiry','DTE','Contracts','Premium','Stock Price','Value'].map(h => (
                ...
                return (
                  <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${p.type === 'CSP' ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}>
                        {p.type}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 tabular-nums text-slate-200">{currentPrice ? fmt$(currentPrice) : '—'}</td>
                    <td className="px-4 py-3 tabular-nums font-semibold text-emerald-400">{fmt$(posValue)}</td>
                  </tr>
                )
              })}
            </tbody>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {STOCK_POSITIONS.map((p, i) => {
                ...
                const pnlDollar = current != null ? (current - p.entryPrice) * p.shares : null
                const pnlPct    = current != null ? ((current - p.entryPrice) / p.entryPrice) * 100 : null
                const color     = pnlDollar == null ? 'text-slate-400' : pnlDollar >= 0 ? 'text-emerald-400' : 'text-red-400'
                return (
                  <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                    <td className="px-4 py-3 font-bold text-slate-100">{p.ticker}</td>
                    ...
                    <td className={`px-4 py-3 tabular-nums font-semibold ${color}`}>{pnlDollar != null ? fmt$(pnlDollar) : '—'}</td>
      ...
      <div className="bg-slate-800/70 backdrop-blur-sm rounded-xl ring-1 ring-white/[0.06] overflow-hidden">
        <div className="px-5 py-4 border-b border-white/[0.05] flex items-center justify-between">
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">Trade Log</p>
          <div className="flex gap-3 text-xs">
            <span className="text-slate-400">
              {TRADE_LOG.length} trades ·{' '}
              <span className="text-emerald-400">{TRADE_LOG.filter(t => t.result === 'win').length}W</span>
              {' / '}
              <span className="text-red-400">{TRADE_LOG.filter(t => t.result === 'loss').length}L</span>
            </span>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.05]">
                {['Ticker','Strategy','Entry','Exit','P&L','Result'].map(h => (
